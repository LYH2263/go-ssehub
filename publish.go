...
func (r *Room) Publish(ctx context.Context, name string, data []byte) (event.Event, error) {
        if err := ctx.Err(); err != nil { return event.Event{}, err }
        r.mu.Lock()
        defer r.mu.Unlock()
        if r.closed { return event.Event{}, ErrClosed }
        if r.Enc == nil { return event.Event{}, fmt.Errorf("%w: encoder", ErrInvalid) }
        ...
        for _, s := range r.Subs {
                if err := s.Q.Push(frame); err != nil {
                        if first == nil {
                                first = fmt.Errorf("%w: %v", backpressure.ErrFull, err)
                        }
                }
        }
        return ev, first
